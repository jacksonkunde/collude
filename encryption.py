...
class Encryptor:
    def __init__(self, n: int, model, model_name: str, device: str, vocab_type: str, seed: int = None, save_mappings: str = None, load_mappings: str = None):
        self.seed = seed
        if seed:
        ...
        if n in [2, 3, 4, 5]:
            if load_mappings:
                ...
                self.tensor_mapping, self.mapping, self.reverse_encryption, self.n_digit_encoding, self.reverse_n_digit_encoding = self._n_digit_help()
        else:
    ...
    def encrypt(self, start: string, secret_message: string, topk: int, num_show: int = 2):
    ...
    def fastest_topk(self, start: str, secret_message: str, topk: int = 2, encoded: bool = False):
    ...
    def _n_digit_help(self):
        ...
        return tensor_mapping, mapping, reverse_encryption, n_digit_encoding, reverse_n_digit_encoding
    ...
    def compute_generation_with_prob(self, start, len_secret_message, temperature=1.0):
        ...
        output = self.model.generate(input_ids, max_length=max_length + input_ids.size(1), temperature=temperature, do_sample=True, top_k=50, top_p=0.92, early_stopping=False)
        ...
        return output, probs[0, torch.arange(input_ids.size(1)-1, output.size(1)-1), generated].sum().item()
