...
def map_char_to_token(char_set, token_set, remaining_map):
    ...
    for char in char_set:
        ...
        remaining = set(token_set_shuffled) - set([word for words in mapping.values() for word in words])
...
def generate_five_digit_encoding():
    to_encode = 'abcdefghijklmnopqrstuvwxyz.?-, |' # let | be our end of string character
    five_digit_encoding = {}
    for v in ['0', '1']:
        for w in ['0', '1']:
            for x in ['0', '1']:
                for y in ['0', '1']:
                    for z in ['0', '1']:
                        i = (int(v) * 16 + int(w) * 8 + int(x) * 4 + int(y) * 2 + int(z)) % len(to_encode)
...
def generate_four_digit_encoding():
    ...
    for w in ['0', '1', '2']:
        for x in ['0', '1', '2']:
            for y in ['0', '1', '2']:
                for z in ['0', '1', '2']:
                    i = (int (w) * 27 + int(x) * 9 + int(y) * 3 + int(z)) % len(to_encode)
